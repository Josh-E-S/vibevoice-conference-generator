...
- **Responsible-use gate**: Generate stays locked until the visitor accepts the responsible-use terms shown above it (no impersonation without consent, no deception, harassment, defamation, or illegal content, rights to any uploaded voice, disclosure that the audio is synthetic). The server refuses renders without it and writes one JSON line per acceptance to `data/consent_log.jsonl` (time, IP, user agent, voices used, script hash — never the script) and to the Space logs as `CONSENT {...}`.
